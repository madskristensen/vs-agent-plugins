...
- Dynamic/context-dependent highlighting or custom classification types → MEF classifier (see [vs-editor-classifier](../adding-editor-classifiers/SKILL.md))
- Full IntelliSense, diagnostics, go-to-definition → LSP (see [vs-language-server](../integrating-language-servers/SKILL.md)), which can include a TextMate grammar for colorization
- User-customizable colors for classification items → [vs-fonts-and-colors](../registering-fonts-colors/SKILL.md)
